...
class _tally {
            ...
            try {
                ...
                if (this.config.sync == 'incremental') {
                    ...
                    if (/^(mssql|mysql|postgres)$/g.test(database.config.technology)) {
                        ...
                        const refetchGuidsByTable = new Map<string, string[]>();
                        for (let i = 0; i < lstPrimaryTables.length; i++) {
                            ...
                            try {
                                fs.copyFileSync(
                                    path.join(process.cwd(), `./csv/_diff.data`),
                                    path.join(process.cwd(), `./csv/_diff_${activeTable.name}.dump`)
                                );
                            } catch (dumpErr) {
                            ...
                            let diffRowCount = await database.executeScalar<number>('select count(*) from _diff');
                            let preTableRowCount = await database.executeScalar<number>(`select count(*) from ${activeTable.name}`);
                            ...
                            if (preTableRowCount >= DIFF_COMPLETENESS_MIN_PRE_ROWS &&
                                ...
                                logger.logMessage(
                                    '  WARNING: _diff for %s appears truncated (returned=%s, db_pre=%s, ratio=%s). ' +
                                    'Skipping delete/refetch for this table to avoid mass-delete; next sync will retry.',
                                    activeTable.name, diffRowCount, preTableRowCount,
                                    (diffRowCount / preTableRowCount).toFixed(2));
                                continue;
                            }
                            ...
                            let toDeleteCount = await database.executeScalar<number>('select count(*) from _delete');
                            ...
                            let modifiedRefetchCount = await database.executeScalar<number>('select count(*) from _refetch');
                            ...
                            let refetchCount = await database.executeScalar<number>('select count(*) from _refetch');
                            let insertedRefetchCount = refetchCount - modifiedRefetchCount;
                            logger.logMessage('  _refetch for %s: %s rows (modified=%s inserted=%s)',
                                activeTable.name, refetchCount, modifiedRefetchCount, insertedRefetchCount);
                            ...
                            if (rewriteDetectionEnabled &&
                                ...
                                logger.logMessage(
                                    '  WHOLESALE REWRITE fast path for %s: refetch=%s, pre=%s (%s%%) — truncate-and-reload',
                                    activeTable.name, refetchCount, preTableRowCount,
                                    ((refetchCount / preTableRowCount) * 100).toFixed(1));
                            }
     ...
     */
    refetchPrimaryTable(activeTable: tableConfigYAML,
                        refetchGuidsByTable: Map<string, string[]>,
                        configTallyXML: Map<string, any>): Promise<void> {
        return new Promise<void>(async (resolve, reject) => {
            const baseFilters: string[] = Array.isArray(activeTable.filters) ? [...activeTable.filters] : [];
            try {
                if (activeTable._fullRefetch) {
                    ...
                    await database.bulkLoad(
                        path.join(process.cwd(), `./csv/${activeTable.name}.data`),
                        activeTable.name,
                        activeTable.fields.map(p => p.type)
                    );
                    ...
                    if (refetchGuids.length === 0) {
                        ...
                        logger.logMessage('  refetched %s rows for %s in %s batch(es)',
                            refetchGuids.length, activeTable.name, batches.length);
                    }
                }
                resolve();
            } catch (err) {
